...
brain_top <- read.delim("Test_fly_lists/Fly_atlas_brain_top.tsv", stringsAsFactors = F, header = F)
...
#Doing 100 replicates 
system.time({
  complete <- foreach(i=1:100, .inorder = F, .packages = "data.table") %dopar% {
    compare_methods(tlist = brain_top, X = pergene$reclassified)
  }
})

stopCluster(mycluster)

save(complete, file="brain_100rep_testdoPar.Rdata")
