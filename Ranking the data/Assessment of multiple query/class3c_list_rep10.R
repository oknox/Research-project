...
class3c_top <- read.delim("Class_IIIc_22hrs.tsv", stringsAsFactors = F, header = F)
colnames(class3c_top) <- c("Secondary_identifier", "Gene_symbol", "GeneDB_Identifier", "Organism_name")
...
system.time({
  for(i in 1:10){
    complete[[i]] <- compare_methods(tlist = class3c_top, X = pergene$reclassified)
  }
})

save(complete, file="class3c_10rep.Rdata")