...
head_top <- read.delim("fly_atlas_head_top.tsv", stringsAsFactors = F, header = F)
colnames(head_top) <- c("Secondary_identifier", "Gene_symbol", "GeneDB_Identifier", "Organism_name")
...
system.time({
  for(i in 1:10){
    complete[[i]] <- compare_methods(tlist = head_top, X = pergene$reclassified)
  }
})

save(complete, file="head_10rep.Rdata")