...
imm_top <- read.delim("immunity_genes_list.tsv", stringsAsFactors = F, header = F)
colnames(imm_top) <- c("Secondary_identifier", "Gene_symbol", "GeneDB_Identifier", "Organism_name")
...
system.time({
  for(i in 1:10){
    complete[[i]] <- compare_methods(tlist = imm_top, X = pergene$reclassified)
  }
})

save(complete, file="imm_10rep.Rdata")