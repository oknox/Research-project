...
#Get vector of test lists 
files <- paste0("Test_fly_lists/", list.files("Test_fly_lists/"))

#Run over all the test lists 
system.time({
  all_lists <- lapply(files, function(x) {
    #Read in file
    t <- read.delim(x, stringsAsFactors = F, header = F)
    colnames(t) <- c("Secondary_identifier", "Gene_symbol", "GeneDB_Identifier", 
                     "Organism_name")
    #Apply function
    complete <- list()
    for(i in 1:10){
      complete[[i]] <- compare_methods(tlist = t, X = pergene$reclassified)
    }
  })
})

save(all_lists, file="All_test_lists_10rep.Rdata")
print(paste("Order of files went through:", files))
