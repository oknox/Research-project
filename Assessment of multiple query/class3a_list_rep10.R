...
class3a_top <- read.delim("Class_IIIa_13_14hrs.tsv", stringsAsFactors = F, header = F)
colnames(class3a_top) <- c("Secondary_identifier", "Gene_symbol", "GeneDB_Identifier", "Organism_name")
...
system.time({
  for(i in 1:10){
    complete[[i]] <- compare_methods(tlist = class3a_top, X = pergene$reclassified)
  }
})

save(complete, file="class3a_10rep.Rdata")